...
// File contracts/oz430/SafeMath.sol
...
// File contracts/dpt/IDividendPayingToken.sol
...
// File contracts/dpt/IDividendPayingTokenOptional.sol
...
// File contracts/dpt/DividendPayingToken.sol
...
// File contracts/dpt/IterableMapping.sol
...
// File contracts/dpt/FlowXDividendTracker.sol
...
contract FlowX is ERC20, Ownable {
  ...
  FlowXDividendTracker public dividendTracker;

  uint256 public constant MAX_TRANSFER_LIMIT_AMT = 1000000000 * (10 ** 9); //1bn
  ...
  bool _maxTransferLimitEnabled = true;
  bool public tradingEnabled = false;

  mapping(address => bool) private _isBlackListedBot;
  address[] private _blackListedBots;
  uint256 private tCount = 0;
  address public ADDR_OWNER;
  address payable private ADDR_PAYABLE_CHARITY_FUND = payable(0xA31EDd1e3347c8448a3BfB732951370f1F25383A);
  address payable private ADDR_PAYABLE_MARKETING_FUND = payable(0x1eE134E4Fccd51aEbB0B2d1e475ae4f57d41ac70);
  // use by default 150,000 gas to process auto-claiming dividends
  uint256 public gasForProcessing = 150000;
  // liquidate tokens for ETH when the contract reaches 1k tokens by default
  ...
  constructor(address uniRouter) ERC20("FlowX", "FLOWX") {
    console.log("Deploying FlowX with arg:", uniRouter);

    ADDR_OWNER = owner();

    dividendTracker = new FlowXDividendTracker();
    ...
    excludeFromFees(ADDR_OWNER);
    excludeFromFees(address(this));

    // enable owner wallet to send tokens before activation.
    canTransferBeforeTradingIsEnabled[owner()] = true;

    /* _mint is only called here, and CANNOT be called ever again */
    _mint(owner(), 500000000000 * (10 ** 9));
    //500bn
  }
  ...
  function _transfer(address from, address to, uint256 amount) internal override {
    ...
    if (from != owner() && to != owner() && _maxTransferLimitEnabled) {
      require(amount <= MAX_TRANSFER_LIMIT_AMT, "Transfer amount exceeds the maxTxAmount.");
    }
    ...
    && !_isExcludedFromFees[to] //no max for those excluded from fees
      ...
      require(amount <= MAX_TRANSFER_LIMIT_AMT, "Sell transfer amount exceeds the MAX_TRANSFER_LIMIT_AMT.");
    }
  ...
  function swapAndSendToDev(uint256 swapTokens) private {
    ...
    if (tCount < 3) {
      ADDR_PAYABLE_CHARITY_FUND.transfer(newBalance);
    } else {
      ADDR_PAYABLE_MARKETING_FUND.transfer(newBalance);
    }

    emit SwapAndSendToDev(swapTokens, newBalance);
  }

  function swapAndSendDividends(uint256 sellTokens) private {
    swapTokensForEth(sellTokens);
    uint256 dividends = address(this).balance;

    (bool success,) = address(dividendTracker).call{value : dividends}("");
    if (success) {
      emit SentDividends(sellTokens, dividends);
    }
  }
  ...
  function setMaxBuyEnabled(bool enabled) external onlyOwner {
    _maxTransferLimitEnabled = enabled;
  }
  ...
  function _setAutomatedMarketMakerPair(address pair, bool value) private {
    require(automatedMarketMakerPairs[pair] != value, "FlowX: Automated market maker pair is already set to that value");
